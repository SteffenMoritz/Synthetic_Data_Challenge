...
# ACS IPSO 2
load("~/GitHub/Synthetic_Data_Challenge/results/fg_ipso_regsdc_conf_from_INCTOT_HHWTconf.rda")
synd <- as.data.frame(h_dat_acs(fg_ipso_regsdc_conf_from_INCTOT))
...
results_acs_IPSO2 <- list(
  ...
  cp1_1 = corrplot(cor(as.data.frame(lapply(orig[,names(synd)%in%acs_num_bin], function(x)as.numeric(as.character(x))))), method = "color", type = "lower"),
...
save(list = c("results_acs_IPSO2"), file = paste0(getwd(), "/ACS_IPSO_INCTOT_HHWTconf/results_acs_IPSO2_2.RData"))
