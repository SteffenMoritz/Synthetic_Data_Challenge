...
###  Distributional comparison of synthesised data

Das ist sehr gut (R-Package: synthpop) by (S_)pmSE
...
hier vlt weiterert text

```{r, echo = FALSE, warning=FALSE, message=FALSE}
result$comp$plots
```

### Hier geht es mit comp weiter ... 
...
hier evntuell weiter text
```{r, echo = FALSE, warning=FALSE, message=FALSE}
library("corrplot")
par(mfrow=c(1,2))
corrplot(result$cp1$corr, method = "color", type = "lower", main = "Original")
corrplot(result$cp2$corr, method = "color", type = "lower", main = "Synthetic")
```
...
print(paste0("Die wichtige Metrik: ", mean(result$ks > 0.05)))
```
