---
title: ACS Dataset Evaluation Report - IPSO v1
...
# Load Source Dataset
load(here::here("results/fg_ipso_regsdc_conf_from_INCTOT.rda"))
load(here::here("Evaluation_ACS_IPSO_INCTOT/results_acs_IPSO1_2.RData"))
#load(here::here("acs.RData"))
result <- results_acs_IPSO1
```
# Executive Summary

According to our utility measures, simulated data using the IPSO approach is not suitable way to generate synthetic data from the ACS dataset. The reader should also be aware, that only a part of this dataset is synthetic. Hence, a high utility for the original variables should not be surprising. The utility measures for the synthetic part are not supporting the usage. The S_pMSE for tables is usually high for the synthetic part. Also the absolute difference in densities and the Bhattacharyya are not supporting a high utility accordingly. Mlodak's information loss criterion underpinns the overall impression.
...
library("corrplot")
par(mfrow=c(1,2))
corrplot(results_acs_IPSO1[[6]], method = "color", type = "lower")
corrplot(results_acs_IPSO1[[7]], method = "color", type = "lower")
```
