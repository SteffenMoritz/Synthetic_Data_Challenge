...
load(here::here("Evaluation_ACS_SYND_SIMP/results_acs_synd_2.RData"))
load(here::here("acs.RData"))
result <- results_acs_sat_mnorm_simple
original <- as.data.frame(ACS)
original$X <- NULL
original$sim_individual_id <- NULL
original$INCTOT <- NULL
synthetic <- as.data.frame(pums_syn_syndatcart)
```
# Executive Summary
...
There is a distinct limited usability according to Mlodak's information loss criterion. Overall, we see mediocre results regarding the utility measures and practically no improvement compared to the **simpler multivariate normal distribution**.
...
generate_uniques_for_acs <-function(df_orig, df_synth, exclude = c("INCWAGE","INCWELFR", "INCINVST", "INCEARN", "POVERTY", "DEPARTS", "ARRIVES")){
...
generate_uniques_pp_for_acs <-function(df_orig, df_synth,identifiers = 1:which(names(df_orig)=="WORKEDYR"),  p = 0.05){
  ...
  count_disclosure <- df %>%
    ...
    filter(INCWAGE_diff < p | INCWELFR_diff < p | INCINVST_diff < p
...
library("corrplot")
par(mfrow=c(1,2))
corrplot(result[[6]], method = "color", type = "lower")
corrplot(result[[7]], method = "color", type = "lower")
```
