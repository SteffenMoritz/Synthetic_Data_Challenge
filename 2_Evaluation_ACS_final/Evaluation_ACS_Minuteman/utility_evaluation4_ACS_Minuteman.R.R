...
load("pums.RData")
load("~/GitHub/SynDatCh/results/pums_syn_syndatcart.RData")
...
# ACS Minuteman
load("~/GitHub/SynDatCh/results/sm_acs_minuteman.rda")
synd <- as.data.frame(h_dat_acs(as.data.frame(sm_acs_minuteman)))
...
c1 <- names(synd[1:ncol(synd)%in%c1a])[-1]
...
orig <- orig[sample(1:1035201, size = 1017704),]
...
results_acs_minute <- list(
  ...
  comp4 = compare(synd[c4], orig[c4], var = names(synd[c4])),
  #comp5 = compare(synd[c5], orig[c5], var = names(orig[c5])),
  
  corr1 <- cor(as.data.frame(lapply(orig[,names(orig)[names(synd)%in%acs_num_bin][-19]], function(x)as.numeric(as.character(x))))),
  corr2 <- cor(as.data.frame(lapply(synd[,names(orig)[names(synd)%in%acs_num_bin][-19]], function(x)as.numeric(as.character(x))))),
...
save(list = c("results_acs_minute"), file = paste0(getwd(), "/Evaluation_ACS_Minuteman/results_acs_minute_2.RData"))
