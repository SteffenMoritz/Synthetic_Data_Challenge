...
# Load Source Dataset
load(here::here("results/sm_acs_minuteman.rda"))
load(here::here("Evaluation_ACS_Minuteman/results_acs_minute_2.RData"))
load(here::here("acs.RData"))
result <- results_acs_minute
original <- as.data.frame(ACS)
original$PUMA <- NULL
original$X <- NULL
synthetic <- as.data.frame(sm_acs_minuteman)
synthetic[,1] <- NULL
```
# Executive Summary

At a first glance, Minuteman does not produces useful marginal distributions for "ACS"-data. This first negative impression is supported by the the Pearson correlation coefficients for binary and (semi-)continuous variables, which are often close to zero. The S_pMSE shows extreme values throughout. Also the absolute difference in densities and the Bhattacharyya distance are not supporting a high utility although they show good results for particular two-way cells. The synthetic dataset is also according to Mlodak's information loss criterion not useful.  
...
library("corrplot")
par(mfrow=c(1,2))
corrplot(results_acs_minute[[5]], method = "color", type = "lower")
corrplot(results_acs_minute[[6]], method = "color", type = "lower")
```
